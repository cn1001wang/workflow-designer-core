...
interface IVerifyNodeInfo {
  verifierChooseType: UserChooseType // 审批人选择类型
...
interface ICCNodeInfo {
  ccChooseType: UserChooseType // 抄送人选择类型
    ...
    switch (nodeType) {
      case NodeType.审批人:
        this.verifyNodeInfo = {
          ...
          verifierChooseType: verifyNodeInfo?.verifierChooseType ?? UserChooseType.发起人自选,
          value: verifyNodeInfo?.value ?? [],
          required: verifyNodeInfo?.required ?? false,
          depth: verifyNodeInfo?.depth ?? 1,
        }
        break
      case NodeType.分支:
